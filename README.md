# 67 Cipher Website

This is a brainrotted encoder/decoder for the 67 Cipher.

## How to use
1. Open `src/index.html` in your browser.
2. Type text to encode or decode.
3. Click the buttons.

## Customization
To get the full "six seven kid" experience:
1. Find an image of the "six seven kid" (or any other brainrot image you want).
2. Save it as `bg.jpg` in the `src` folder.
3. Open `src/style.css` and change the background URL:
   ```css
   background-image: url('bg.jpg'), linear-gradient(...);
   ```

## Cipher Logic
- **Substitution Cipher**: Maps alphanumeric characters (0-9, A-Z) to 36 specific Unicode characters that look like **6** or **7**.
- **Character Set**: Uses exactly 18 variants of "6" and 18 variants of "7".
- **Case Insensitive**: 'A' and 'a' map to the same symbol.
- **Preserves**: Spaces, punctuation, and special characters are kept as is.
- **Visuals**: The output looks like a mix of 6s and 7s (e.g., `6`, `７`, `б`, `T`, `b`, `Γ`).
